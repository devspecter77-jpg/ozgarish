...
import { Phone, Lock, Eye, EyeOff, ArrowLeft, Shield } from 'lucide-react'
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (err === 'not_registered') {
      setServerError("Bu Google hisob ro'yxatdan o'tmagan. Avval ro'yxatdan o'ting.")
    }
  }, [])
  ...
  const submit = async (e) => {
    ...
    if (result === true) {
      ...
      setServerError(result || 'Kirish amalga oshmadi')
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#0F0A1E] flex items-center justify-center px-4 py-8">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card">
          <h2 className="text-xl font-bold mb-6">Kirish</h2>
          ...
          {/* Telegram bot eslatmasi */}
          <div className="bg-[#229ED9]/10 border border-[#229ED9]/30 rounded-xl p-4 mb-4">
            <p className="text-sm text-white/80 mb-3 font-medium">
              📱 Kirish uchun avval Telegram botga <b>/start</b> bering
            </p>
            <p className="text-xs text-white/40 mb-3">
              Bot sizga har kuni motivatsiya va eslatmalar yuboradi
            </p>
            <a
              href="https://t.me/ozgarish_mukammallik_bot?start=login"
              target="_blank"
          ...
          <form onSubmit={submit} className="space-y-4" noValidate>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-white/10 border rounded-xl pl-10 pr-4 py-3 text-white placeholder-white/30 outline-none transition-colors ${
                    errors.phone ? 'border-red-500/60 focus:border-red-500' : 'border-white/20 focus:border-primary'
                  }`}
                />
              </div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full bg-white/10 border rounded-xl pl-10 pr-11 py-3 text-white placeholder-white/30 outline-none transition-colors ${
                    errors.password ? 'border-red-500/60 focus:border-red-500' : 'border-white/20 focus:border-primary'
                  }`}
                />
            ...
            <button type="submit" disabled={loading} className="animated-button mt-2">
              ...
              <span className="text">{loading ? 'Kirilmoqda...' : 'Kirish'}</span>
          ...
          <p className="text-center text-white/40 text-sm mt-5">
            Hisobingiz yo'qmi?{' '}
            <Link to="/register" className="text-primary-light hover:underline">Ro'yxatdan o'ting</Link>
          </p>
          ...
          <a href="#" onClick={e => { e.preventDefault(); window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/auth/google` }}
